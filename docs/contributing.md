...
- __Files__
  ...
  - With the exception of Containers and Components, which should be `PascalCase` - eg. `ProfileView.js`
- __Directories__
...
- If there's more than 1 file in a directory that are related, group them within their own directory
  - eg. if I have 2 components: `/components/RecipeListing.js` and `/components/ProfileView.js`, create a new directory within components called `recipes` and put the 2 files within (removing `Recipes`). The result would be: `/components/recipes/Listing.js` and `/components/recipes/View.js`
